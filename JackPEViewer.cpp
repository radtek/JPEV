...
TCHAR       g_szAppName[]       = _T("JACK_PEVIEWER_FRAME");
TCHAR       g_szTitle[]         = _T("Jack Portable Executable Viewer");
...
int CreateMapFile( _tstring strFileName
                      , HWND hWnd
            ...
            case ID_FILE_OPEN:
				{
                    ...
                    if(pNikPEViewer)
                    {
                        ...
                        if(!strFileName.empty())
                        {
                            ...
                            CreateMapFile( strFileName, hWnd, pNikPEViewer );
	...
	case WM_DROPFILES:
			{
                ...
                CNikPEViewer * pNikPEViewer = new CNikPEViewer;
                if(pNikPEViewer)
                    CreateMapFile( strFile, hWnd, pNikPEViewer );
			}
			break;
	case WM_LBUTTONDOWN:
		break;
...
)
{
    ...
    if( ! hPrevInstance )
    {
        ...
        wc.hIcon         = LoadIcon( hInstance, MAKEINTRESOURCE(IDI_JACKPEVIEWER) );
		wc.hCursor       = LoadCursor(NULL, IDC_ARROW);
		wc.hbrBackground = 0;
		wc.lpszMenuName  = MAKEINTRESOURCE(IDC_JACKPEVIEWER);
        ...
        && lpszCmdLine[0])
    {
        ...
        CNikPEViewer * pNikPEViewer = new CNikPEViewer;
        if(pNikPEViewer)
            CreateMapFile( strComandLine, g_hWndMain, pNikPEViewer );
    }
