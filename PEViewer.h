...
public:
    ...
    {
        if (nShow) {
            SetWindowPos(m_cPEVImportList.GetWindowHandle(), HWND_TOP
                , 0, 0, 0, 0, SWP_NOMOVE | SWP_SHOWWINDOW | SWP_NOSIZE);
            //ChangeColor(m_cPEVImportList.GetWindowHandle());
        }
        else
        {
